...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        for act in self.getLegalActions(state): 
          count[act] = self.getQValue(state, act)  
        return count[count.argMax()]  #Getting max value from count
    ...
    def computeActionFromQValues(self, state):
        ...
        bestact = []
        for act in self.getLegalActions(state):  
            if bestval == self.getQValue(state, act):
                bestact.append(act)
    ...
    def getAction(self, state):
        ...
        if util.flipCoin(self.epsilon):  #flip coin prob
            ...
            action = self.computeActionFromQValues(state)  #Getting action
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        samp = reward + self.discount * self.getValue(nextState)
        self.qvalues[(state,action)] = (1 - self.alpha) * self.qvalues[(state,action)] + self.alpha * samp  #Q updates
